...
(function (factory) {
     ...
     */
    methods.canvastoDataURL = function (canvas, quality, type) {
     ...
     * @returns {Promise(string)} Promise含有一个dataURL字符串参数
     */
    methods.filetoDataURL = function (file, callback) {
     ...
     * @returns {Promise(Image)}
     */
    methods.dataURLtoImage = function (dataURL, callback) {
        const img = new Image();
        img.onload = function () {
            callback && callback(img);
        };
        img.onerror = function () {
            new Error('dataURLtoImage(): dataURL is illegal');
        };
        img.src = dataURL;
    };
     ...
     * @returns {Promise(Blob)}
     */
    methods.compressAccurately = function (file, config = {}, callback) {
        ...
        methods.filetoDataURL(file, function (dataURL) {
            ...
            methods.dataURLtoImage(dataURL, function (image) {
                const canvas = methods.imagetoCanvas(image, Object.assign({}, config));
                /**
                ...
                for (let x = 1; x <= 7; x++) {
                    ...
                    compressDataURL = methods.canvastoDataURL(canvas, imageQuality, mime);
                ...
                const compressFile = methods.dataURLtoFile(compressDataURL, originalMime);
